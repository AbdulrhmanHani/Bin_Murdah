...
<head>
    ...
    <title>Company name | منشور</title>
</head>
...
<body>
    ...
    <div class="posts-parent">
        <div class="content">
            <div id="posts-child">
                <div class="post-content">
                    ...
                    <span class="date-post">{{ $publication->created_at }}</span>
                    <h4 class="post-text">{{ $publication->publish_title }}</h4>
                    <p class="post-text">{{ $publication->publish_content }}</p>
                    <a href="{{ url('/uploads', $publication->publish_img) }}">
                        <img width="250px" src="{{ url('/uploads', $publication->publish_img) }}" alt="">
                    </a>
                    <hr>
                    <div class="reply">
                        @foreach ($publication->PublicReplies as $reply)
                            <div class="reply">
                                @if ($reply->User->id == auth()->user()->id)
                                    <span class="user-comment2">{{ $reply->User->name }}</span>
                                    <p class="comment-post2">{{ $reply->public_reply_content }}</p>
                                @else
                                    <span class="user-comment">{{ $reply->User->name }}</span>
                                    <p class="comment-post">{{ $reply->public_reply_content }}</p>
                                @endif
                                <a href="{{ url('/uploads', $reply->public_reply_img) }}">
                                    <img width="250px" src="{{ url('/uploads', $reply->public_reply_img) }}"
                                        alt="">
                                </a>
                            </div>
                        @endforeach
        ...
        <form id="form" method="POST" action="{{ url('/public/post', $publication->id) }}"
            enctype="multipart/form-data">
            @csrf
            <button type="submit"><i class="fas fa-paper-plane"></i></button>
            <input autofocus name="replyContent" type="text" placeholder="الرد علي المنشور" />
            <input name="public_reply_img" type="file" id="" />
        </form>
        <!-- End Post -->
        <!-- javaScript -->
